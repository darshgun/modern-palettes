...
test('Chrome renders correctly', () => {
  const tree = renderer.create(<Chrome {...color.red} />).toJSON();
...
test('Chrome onChange events correctly', () => {
  ...
  const tree = mount(<Chrome {...color.red} onChange={changeSpy} />);
...
test('ChromeFields renders correctly', () => {
  const tree = renderer.create(<ChromeFields {...color.red} />).toJSON();
  expect(tree).toMatchSnapshot();
});

test('ChromePointer renders correctly', () => {
  const tree = renderer.create(<ChromePointer />).toJSON();
  expect(tree).toMatchSnapshot();
});

test('ChromePointerCircle renders correctly', () => {
  const tree = renderer.create(<ChromePointerCircle />).toJSON();
  expect(tree).toMatchSnapshot();
});

test('Chrome renders custom styles correctly', () => {
  const tree = renderer
    .create(<Chrome styles={{ default: { picker: { boxShadow: 'none' } } }} />)
    .toJSON();
  expect(tree.props.style.boxShadow).toBe('none');
});

test('Chrome renders correctly with width', () => {
  const tree = renderer.create(<Chrome width={300} />).toJSON();
